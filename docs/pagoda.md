...
- Types: the binding can be annotated (`let x: i32 = ...`) or inferred from the initializer; supported names are `i64`/`int`, `i32`, `bool`, `string`, or a struct name.
- Lowering:
...
- Grammar: `Stmt -> 'return' Expr | 'let' Ident (':' Ident)? '=' Expr | Expr`.
...
- Types: params/returns may be annotated as `i64`/`int`, `i32`, `bool`, `string`, or a struct name; otherwise they are inferred as `i64`. Arity must match.
